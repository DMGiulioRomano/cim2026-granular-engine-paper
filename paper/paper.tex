...
deviazione per-grano (\S\ref{sec:deviazione}); la molteplicità e
l'indipendenza delle voci (\S\ref{sec:voci}). Chiude la sezione il
...
La Fig.~\ref{fig:stream-minimo-waveform} verifica
...
\begin{figure}[h]
    \includegraphics[width=\linewidth]{examples/ex5_esempio1/ex5_esempio1_comparison}\\
    \captionof{figure}{Confronto fra il sorgente (originale) e la forma d'onda
    prodotta dal rendering dello stream minimo (elaborato).}\label{fig:stream-minimo-waveform}
\end{figure}
