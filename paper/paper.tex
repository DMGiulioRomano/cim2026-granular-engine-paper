...
\usepackage{titlesec}
\titleformat{\section}{\normalfont\fontsize{12}{14}\bfseries\centering}{\thesection.}{0.5em}{}
...
Il sistema adotta tempo differito oggi in una fase storico-informatica in cui il tempo reale 
...
arricchendolo di un vocabolario e di una semantica propri del dominio. 
  ...
  (DSL) in cui ogni parametro
  è dichiarato come intenzione, validato da un \emph{Language Server}\footnote{Un \emph{Language Server} è un componente software che analizza 
...
Tra stream e grano si colloca la \emph{voce}, che ho posizionato lateralmente. 
Da un punto di vista mereologico lo stream 
è composto di grani, mentre la voce è un modo di partizionare i grani all'interno
di uno stream. La voce rappresenta un
insieme di offset (su \emph{pitch}, \emph{onset}, \emph{pointer} e \emph{pan}) che il
\texttt{VoiceManager} calcola e fonde in ciascun grano al
momento della generazione (\S\ref{sec:multivoce}). Nello \texttt{Stream} le voci sono 
...
In questo paragrafo spiegherò brevemente come dal DSL
 si passa alla generazione 
dei grani. Ogni Stream è pensato come una struttura dati dizionario popolato da chiavi (\emph{keywords}). Queste vengono
lette, validate ed trasformate dal \texttt{ParameterOrchestrator} in una rappresentazione intermedia\footnote{Il termine intermediate
 ...
 dal formato di output, su cui operano le trasformazioni.} composta da oggetti \texttt{Parameter} 
tipizzati e valutabili nel tempo. In questo stato essi sono oggetti "dormienti" perché
contengono il valore del parametro ma non lo hanno ancora emesso per generare alcun grano.
%È su questa IR che operano i controller, interrogandola istante per istante per emettere gli eventi.

\begin{figure}[t]
  \centering
  \input{figures/param-orchestrator.tikz}
\caption{pino}
\end{figure}

Partiremo dalla dichiarazione di un singolo stream
attraverso un frammento di un \emph{file.yml}. Si può osservare 
nel listato~\ref{lst:stream-minimo}
\lstinputlisting[
  language=,
  basicstyle=\ttfamily\fontsize{7.5}{9}\selectfont,
  label=lst:stream-minimo,
  caption={}
]{examples/ex5_esempio1/ex5_esempio1.yml}
Queste quattro \emph{keywords} sono le sole obbligatorie: fissano l'identità
