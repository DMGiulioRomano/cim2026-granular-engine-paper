...
da una fase \emph{imperativa} (il campionamento
...
La seconda materializza quella dichiarazione: poche chiavi 
YAML si espandono in migliaia di eventi granulari, ciascuno 
calcolato al proprio tempo $t$.

Si descrive di seguito 
l'architettura dal basso: dal \texttt{Grain} alla IR 
che ne governa la generazione.
...
Ogni Stream è pensato come una struttura dati dizionario popolato da chiavi (\emph{keywords}). Queste vengono
lette, validate ed trasformate dal \texttt{ParameterOrchestrator} in una rappresentazione intermedia. composta da oggetti \texttt{Parameter} 
