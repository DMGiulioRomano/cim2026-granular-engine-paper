...
is explicit, legible parametric control over that mass of data. Parameters are 
declared in a YAML domain-specific language — validated during writing by a 
...
reale è tecnicamente disponibile; sulle ragioni di questa scelta
...
\begin{itemize}
\item un linguaggio di dominio testuale (DSL YAML) in cui ogni parametro
  è dichiarato come intenzione, validato da un Language Server durante
  la scrittura e interpretato come \emph{tendency mask} (traiettoria
  centrale, range di deviazione, campionamento per-grano);
\item un sistema multi-voce dichiarativo che differenzia le voci lungo
  quattro assi ortogonali (pitch, onset, pointer, pan) mediante
  strategie intercambiabili (accordo, lineare, stocastico, \dots) e un
  parametro \texttt{scatter} che controlla l’indipendenza temporale
  delle griglie;
\item una partitura grafica generata automaticamente, il cui asse
  ...
  e un workflow STEMS con cache incrementale ed export verso DAW,
  che chiude il ciclo di retroazione.
\end{itemize}
