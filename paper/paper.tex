...
accettare uno scalare oppure un inviluppo $f(t)\to v$ definito a tratti
su breakpoint, con interpolazione lineare, cubica (tangenti
Fritsch-Carlson, monotona e senza overshoot) o a gradino; un formato
compatto genera $N$ ripetizioni di un pattern, la cui distribuzione
temporale può essere uniforme, esponenziale (\emph{accelerando}) o
logaritmica (\emph{ritardando}). L'inviluppo sostituisce lo scalare
ovunque il parser lo accetti: dalla densità alla traiettoria del pointer
alle dimensioni delle strategie multi-voce.

Ogni \texttt{Parameter} è composto da una traiettoria centrale $c(t)$
...
\begin{figure}[t]
  ...
  \caption{Partitura dell'esempio \texttt{ex3\_voices} (20~s): cinque
    ...
    deterministiche: output bit-identico fra run.}\label{fig:voices}
\end{figure}
