...
Il terzo motivo è una conseguenza del primo. La generazione dei grani e la loro
resa sonora sono due fasi separate: la prima interpreta la specifica YAML e produce
una lista di oggetti, ciascuno con tutti i suoi parametri.
Questa è a tutti gli effetti una rappresentazione intermedia, una descrizione astratta
e agnostica al modulo che la processerà. Oggi la stessa lista alimenta
due motori di rendering audio, uno interno in NumPy e uno che genera una event-list per Csound, e tre
esportazioni: un progetto Reaper (file .rpp); una sessione Sonic Visualiser; un export JSON. 
