...
|----|-----------|---------|-----------------|
| D1 | `dephase` → `jitter`? — **APERTA, in discussione in sessione dedicata** (handoff 2026-08-12); il verdetto torna qui | (a) tenere la chiave, glossarla alla prima occorrenza («jitter probabilistico») e giustificare in lettera; (b) rinominare nel paper e nel PGE; (c) rinominare solo nel paper | Nessun DOI Zenodo ancora pubblicato (audio su OSF view-only) → il rename è tecnicamente aperto. Il costo è la divergenza paper/codice: `dephase` è chiave YAML reale del PGE. R1 insiste due volte (M5, D24) e propone anche `offset_probability` sotto `pointer` (D19) |
