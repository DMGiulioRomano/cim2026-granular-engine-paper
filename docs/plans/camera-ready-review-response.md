...
| D2 | Sorte del differito in «implicazioni» (sezione rimossa) — **DECISA 2026-08-12: (a) restringere e concedere · RIAPERTA da D7** | (a) restringere + concedere la distinzione motore/descrizione, tenendo solo la parte difendibile (la MAP come rappresentazione totale post-render); (b) tagliare (R1); (c) espandere la giustificazione (R2) | Concedere a R1.M2 che RT/DT è proprietà del motore, non della descrizione; tenere solo ciò che il differito abilita davvero. Risponde anche a R2.3 e libera spazio. Vincolo maestro: mai «real-time come cambio di paradigma». **Riaperta da D7:** se il motore diventa l'oggetto dichiarato del paper, la concessione «il motore non è oggetto dell'articolo» (R1.M2) non è più disponibile nella forma decisa il 12 ago. Le due si chiudono insieme, e la lettera deve raccontare la scelta senza contraddirsi |
| D3 | Dove risponde l'obiezione «≈ patch Max/PD» — **CHIUSA 2026-08-13: nessuna delle tre. Si ritira la rivendicazione che la provoca** | (a) in introduzione; (b) in «tradizione» (sezione rimossa, confluita in `sec:conclusioni`); (c) nel riquadro contributi | Non si risponde all'obiezione: si toglie il confronto. Spariscono (i) ogni claim di superiorità del modello dichiarativo su altri sistemi e (ii) ogni asserzione su *come si lavora* con Max, Pure Data, SuperCollider, Csound. Motivo: l'affermazione è confutabile da chiunque con «tu non sai come lavoro io» — la maggior parte dei compositori usa quei sistemi in modo di fatto dichiarativo. **Criterio che sostituisce D3, da applicare in tutto il paper:** le affermazioni positive su ciò che *questo* sistema fa restano; le affermazioni su come altre persone lavorano con altri strumenti vanno via. **Non tocca la genealogia**: i precursori citati con la loro tecnologia (CMask con uscita Csound, Lippe su ISPW, GrainLab, EC2) restano — sono storia, non confronto. Risponde a R1.M1 e R1.D29 per concessione piena |
...
| D4 | Specifica completa del linguaggio — **DECISA 2026-08-12: (d) albero dentro un listato · ATTUATA 2026-08-17** | (a) tabella compatta; (b) rinvio a documentazione repo; (c) tabella + rinvio; **(d) albero della grammatica (AST/tree) reso come `lstlisting`** | Un albero indentato in `lstlisting` costa meno spazio di una tabella e mostra la ramificazione proprietà → sottoproprietà → dominio, che è esattamente ciò che R1.M6 chiede. **Forma scelta 2026-08-17: stile YAML** (`chiave: <dominio>`), non albero con connettori — resta nella notazione che il paper insegna, riusa lo stile `lstlisting` già in uso, e i connettori costavano 4 caratteri per livello in una colonna da ~48. **Scope: completo** (74 righe dopo il diff contro `yaml.md`), niente rinvio al repo: misurato, sta in colonna con margine. **Ingombro verificato** compilando col `cim2026.sty` reale, non stimato |
...
| R1.M2 | La discussione tempo reale/differito non serve: RT/DT è proprietà del *motore*, non della descrizione; il motore non è oggetto dell'articolo; solo la MAP è soggetta alla distinzione, e il suo status è sopravvalutato | Concedere la distinzione motore/descrizione (è corretta e rafforza il resto); ridurre la sezione alla sola parte difendibile | P | D2, «implicazioni» (sezione rimossa) | ☐ |
...
| R1.D25 | Fig. 4: l'andamento cubico 20–80% non si vede (sembra lineare a scalini); deviazione = 100 di cosa? | Verificare la figura contro il YAML; correggere didascalia e unità | A | `24-deviazione` | ☑ didascalia corretta su due fronti: il revisore aveva ragione, l'esempio ha `type: step` e non `cubic` — corretti «cubico» → «a gradini» e «con continuità» → «per gradi». **Unità chiusa in didascalia** (non a monte nel motore, issue #36): spiegato che il `%` in legenda (`ptr dev %`, `pan %`, `grain dur %`) è la probabilità che la deviazione si applichi, non la sua ampiezza — che è invece la curva `ptr dev` senza `%` di Fig.~\ref{fig:deviazione-ab} |
...
| R1.D27 | `pan_range`: jitter sul pan o apertura del fronte? nome cattivo; «come ogni parametro» non torna | Metà chiusa: la regola delle tre chiavi ora si legge **come regola** nel cappello di `sec:architettura`, prima degli esempi, con `pan` come istanza (`c330f3c`) — il revisore non la incontrerà più già applicata. **Resta** la semantica di `pan_range` in sé (jitter sul pan o apertura del fronte?) | A | `20-architettura` ✓, `27-voci` | ◐ 2026-08-17 |
...
| 5 | Figure | D11 chiusa ✓ (riposizionata; densità bassa ottenuta spostando i target della lente `magnify` su un punto a bassa densità). D25 chiusa ✓ (unità disambiguata in didascalia, **issue #36 chiusa**); Fig. 1: D6 ✓, D8 ✓, D7 ◐ (restano click e campioni a cavallo); **figura spettrale nuova (D14, decisa: si fa)** | 0 (D6 ✓) |
