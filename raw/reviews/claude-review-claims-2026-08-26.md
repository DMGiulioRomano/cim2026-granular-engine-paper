...
| C2 | Il valore era sbagliato (misurato −38,1 dB contro i −74 dB dichiarati) e il punto è di specifica del motore, non del paper: tolta l'intera clausola numerica dalla `\notaBande`, resta solo l'elisione qualitativa a fattore 2 | — |
| C4 | `Vaggione2002` sosteneva la metà sbagliata della frase (l'articolo parla di decorrelazione spaziale a 5 µs–1,5 ms, mai di soglia di separazione in eventi). Frase riscritta in due parti: Vaggione per l'attributo spaziale sotto pochi millisecondi, Truax~\cite[p.~18]{Truax1988} per la soglia dei 50 ms oltre cui i grani si separano — verificata sulla scansione della fonte. Contestualmente tolta la frase su Truax e le diciotto voci in `27-voci.tex` (cuciva due meccanismi distinti di Truax 1994 in un gesto solo) | — |
| C5 | «fino a diciotto voci»: confermato su Truax 1994 p. 42, aggiunta la pagina | — |
| B2 | Le due equazioni non erano "riformulazione mia": il modello centro+banda campionata è già esplicito in CMask~\cite{Bartetzki1997} (mappatura lineare fra estremi) e descritto a parole da Truax~\cite[p.~17]{Truax1988}. Inoltre il paper dava per scontato il campionamento uniforme, mentre il motore ha `distribution_mode: uniform \textbar{} gaussian` a livello di stream — verificato su `distribution_strategy.py` e `docs/reference/yaml.md` del PGE. `eq:tendency_mask` ora dichiarata come caso uniforme fra le forme disponibili | — |
| B2 (residuo) | `eq:iot` citava ancora Truax~\cite{Truax1988} nonostante `\notaEq` (B5) attribuisca già $d$ a Roads: incongruenza fra due frasi dello stesso paragrafo, sopravvissuta al giro precedente. Spostata la citazione a Roads~\cite[p.~93]{Roads2001}. Stessa correzione in `10-introduzione.tex:43` (sincrona/quasi-sincrona/asincrona citava `Truax1988,Roads2001` insieme) | — |
| B2 (residuo) | Il termine *inter-onset time* (IOT) in `23-griglia.tex:28` citava Truax 1988, che non lo usa mai (verificato su tutte le 13 pagine: lui parla di *delay time between grains*). Cercata l'origine: **Ross Bencina, «Implementing Real-Time Granular Synthesis», in *Audio Anecdotes III*, A K Peters/CRC Press, 2007**, lo definisce alla lettera («interonset time: the time between the onset of successive grains»), senza attribuirlo a sua volta a nessuno — verosimilmente un prestito dalla letteratura sulla percezione ritmica (*inter-onset interval*). Verificato anche l'indice di *Microsound* (Roads 2001, pp. 404-406): nessuna voce «inter-onset». **Decisione dell'autore: nessuna citazione sul termine** (è una definizione operativa, non un'affermazione da sostenere) — Bencina non aggiunto a `refs.bib` | — |
| B3 | Il termine esatto *granular sampling* compare nel titolo di Lippe~\cite{Lippe1993cim} e da nessun'altra parte nel corpus verificato; Truax~\cite{Truax1990} usa "granulation of stored sampled sound"/"granulation of sampled sound" (procedura affine, dicitura diversa) --- verificato via grep sul PDF. Aggiungere Truax1990 sarebbe la stessa attribuzione impropria corretta altrove in sessione: nessuna modifica, resta solo Lippe1993cim | — |
| B4 | Il testo dice già "al calcolatore", non "la prima sintesi granulare" in assoluto, e cita Roads1978. Xenakis, *Analogique B* (1959), lavorava per taglio/giunzione di nastro, non al calcolatore; `Arcella2012` è un modello al computer del 2012 di quel processo del 1959, non prova che Xenakis usasse un calcolatore. Claim già difendibile: nessuna modifica | — |
| C3 | Verificata la formula esatta del motore in `grain_renderer.py:109-119`/`main.orc`: `mid=cos\theta`, `side=sin\theta`, `L,R=(mid\pm side)/\sqrt2` --- due pickup a figura otto ortogonali, non un canale omni+dipolo. Le due estensioni della nota ("perifonico per $n$ altoparlanti", "basta l'elevazione per l'ambisonics") non erano difendibili: la prima confondeva pantofonico/perifonico (terminologia Gerzon~\cite{Gerzon1973} invertita), la seconda ignorava che manca un canale $W$. Riscritta come rotazione di 45\textdegree{} fra mid/side e L/R, con decodifica pantofonica a microfoni virtuali indipendente dal motore (agnostico sul numero di canali, L/R \`e il default) --- aggiunta `Gerzon1975` in bibliografia per la notazione $x=\cos\theta,y=\sin\theta$ | — |
| C6 | Aggiunta `Roads2001Pulsars` alla caption di `23-griglia.tex:60`. Verificato sul PDF (§2 "Spectra of Basic Pulsar Synthesis", p. 137-138): lo spettro di un treno periodico di grani è la convoluzione fra la forma d'onda del grano e l'inviluppo del treno, con righe spaziate alla frequenza di ripetizione $f_p$ (= densità nel lessico del paper) e sagomate dall'inviluppo; Fig. 6 lo mostra graficamente. `DePoliPiccialli1991` scartata: stesso fenomeno ma angolazione meno diretta (sincronia col periodo di pitch, non spettro del treno) | — |
| B1 | Chiuso per scelta dell'autore (confermata il 2026-08-30): lineage Koenig 1970 → Truax → CMask verificato su fonte primaria (Koenig, *PROJECT 2*, §4.6, pp. 44-48) e documentato in `wiki/concepts/tendency-mask.md`, ma il testo del paper non viene riscritto — l'attribuzione a Truax resta in `10-introduzione.tex` e `24-deviazione.tex` | — |
| C7 | `10-introduzione.tex:16-19`: assoluto ammorbidito in "non consente di norma"; corretto anche un refuso rimasto in sospeso ("...renderizzato da uno / non consente...", frase tronca) | — |
| D (Seeger1958) | Citato in `sec:architettura`, dove il paragrafo sui tre moduli d'esportazione introduce la MAP: YAML come notazione prescrittiva, MAP come notazione descrittiva (documenta il generato, non riletta come input) --- coerente con il nodo "log vs partitura" già in `wiki/concepts/graphic-score.md` | — |
| D (resto: TENOR, RolfeKeller2000, tradizione CIM, Truax1994 in sec:pointer) | Aperto | — |
| F1 (nuovo, trovato 2026-08-30) | `27-voci.tex:56` dice dello `scatter`: «quali voci si sganciano e in quali istanti cambia a ogni esecuzione». Verificato su `stream.py` (submodule v8.0.0): `indep_iot` viene da `self._density.calculate_inter_onset`, che pesca dallo stesso RNG seminato per stream/componente di `density` — a parità di seed dichiarato il calcolo è deterministico, la frase lo nega. Rilevante perché l'abstract si appresta a dichiarare la riproducibilità come tratto di prima pagina. Aperto, non toccato nella sessione dell'abstract/conclusioni (decisione dell'autore, 2026-08-30) | — |
