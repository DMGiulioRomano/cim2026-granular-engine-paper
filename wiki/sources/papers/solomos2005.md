...
- **Loop lungo** ← «*generalized interaction* being *internal to the musical work*» (p. 3) + «*aujourd'hui le compositeur interagit à tout moment*» (via 2003 ent4, qui rievocato p. 13). L'interazione vaggioniana è permanente e interna; PGE la materializza in tempo differito come ciclo iterativo *specifica → ascolto → riscrittura*. Solomos chiarisce che *interaction* per Vaggione non significa real-time gestural ma articolazione écriture↔algorithme — *interaction* è il modo di operare, non la latenza. Cfr. [[interactivity-rate]] per la sintesi di cinque fonti convergenti (inclusa questa) sulla ridefinizione CIM di interattività come rate di feedback.
